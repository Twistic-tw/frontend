...
export function applyAdvancedFilters(rows: Record<string, string>[]): Record<string, string>[] {
  return rows.filter(row => {
    return Object.entries(advancedFilters.value).every(([field, filter]) => {
      ...
      switch (filter?.type) {
        case 'text': return val.includes(filter.value.toLowerCase())
        case 'number': return Number(val) >= filter.min && Number(val) <= filter.max
        case 'date': {
          const dateVal = new Date(val)
          return dateVal >= new Date(filter.start) && dateVal <= new Date(filter.end)
        }
        case 'boolean': return val === (filter.value.toString().toLowerCase())
        default: return true
      }
    })
  })
